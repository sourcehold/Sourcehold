...
bool MapFile::LoadFromDisk(ghc::filesystem::path path) {
  if (!Parser::Open(path.string(), std::ifstream::in | std::ios::binary)) {
    Logger::error(PARSERS) << "Unable to open map file '" << path.string()
                           << "'!" << std::endl;
    return false;
  }
  ...
  if (0xFFFFFFFF != Parser::GetDWord()) {
    Logger::error(PARSERS) << "Magic number of map file '" << path.string()
                           << "' is wrong!" << std::endl;
    return false;
  }
...
MapFile::MapSec MapFile::BlastSection() {
  ...
  if (header.compressedLen > MAX_COMPRESSED_LEN) {
    Logger::error(PARSERS) << "Map section exceeds the maximum size of "
                           << MAX_COMPRESSED_LEN << " bytes!" << std::endl;
    return sec;
  }
  ...
  switch (ret) {
    case 2:
      Logger::error(PARSERS) << "PKWARE blast: Out of input!" << std::endl;
      break;
    case 1:
      Logger::error(PARSERS) << "PKWARE blast: Output error!" << std::endl;
      break;
    case -1:
      Logger::error(PARSERS)
          << "PKWARE blast: Literal flag not zero or one!" << std::endl;
      break;
    case -2:
      Logger::error(PARSERS)
          << "PKWARE blast: Dictionary size not in rane 4-6!" << std::endl;
      break;
    case -3:
      Logger::error(PARSERS)
          << "PKWARE blast: Distance is too far back!" << std::endl;
      break;
    default:
      break;
  }
  ...
  if (crc != header.crc32) {
    Logger::warning(PARSERS)
        << "CRC32 checksum mismatch in map section!" << std::endl;
  }
