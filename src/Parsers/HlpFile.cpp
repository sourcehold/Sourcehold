...
bool Parsers::LoadStrongholdHlp() {
  ...
  if(!ws.is_open()) {
      Logger::error(PARSERS) << "Unable to load help file from " <<
  path.string() << std::endl; return false;
  }

  ws.ignore(2); // magic number
  ws.imbue(std::locale(ws.getloc(),new std::codecvt_utf16<wchar_t, 0x10ffff,
  std::little_endian>));
  ...
  for(std::wstring::iterator it = fb.begin(); it != fb.end(); ++it) {
      if(*it == L'<') {
          std::wstring::iterator endOfContents = fb.begin() + fb.find(L'>', it -
  fb.begin());
          ...
          std::wstring ident = boost::algorithm::split(cts, contents,
  boost::algorithm::is_any_of(matchAssign)).at(0); if(ident.at(0) == L'\\') {
              isEndOfBlock = true;
          ...
          if(ident == L"PIC") {
              ...
              boost::algorithm::split(cts, contents,
  boost::algorithm::is_any_of(",= ")); std::wstring num = cts.at(1);
              ...
              boost::algorithm::split(cts, contents,
  boost::algorithm::is_any_of(matchAssign)); std::wstring num = cts.at(1);
  ...
  boost::algorithm::is_any_of(matchAssign)); std::wstring num = cts.at(1);
              ...
              if(isEndOfBlock) {
                  ...
                  // new section created
                  std::wstring name = boost::algorithm::split(cts, contents,
  boost::algorithm::is_any_of("\"")).at(1);
  ...
  boost::algorithm::is_any_of("\"")).at(1);
              ...
              HlpSection sec;
              sec.type = isEndOfBlock ? SectionType::ENDHEADER :
  SectionType::HEADER; currentSec.children.push_back(sec); }else if(ident ==
  L"BODY") { HlpSection sec; sec.type = isEndOfBlock ? SectionType::ENDBODY :
  SectionType::BODY; currentSec.children.push_back(sec); }else if(ident ==
  L"INCLUDE") { std::wstring name = boost::algorithm::split(cts, contents,
  boost::algorithm::is_any_of("\"")).at(1);
              ...
              HlpSection sec;
              sec.type = isEndOfBlock ? SectionType::ENDLINK :
  SectionType::LINK; currentSec.children.push_back(sec); }else if(ident ==
  L"CENTRE") { HlpSection sec; sec.type = isEndOfBlock ? SectionType::ENDCENTRE
  : SectionType::CENTRE; currentSec.children.push_back(sec); }else {
              Logger::warning(PARSERS) << "Unknown comand in help file at pos "
  << it - fb.begin() << std::endl;
          }
      }
  }*/
...
HlpSection *Parsers::GetHlpSection(std::wstring name) {
  // TODO: speed up
  // for(std::vector<HlpSection>::iterator it = _sections.begin(); it !=
  // _sections.end(); ++it) {
