...
bool Gm1File::LoadFromDisk(ghc::filesystem::path path, bool cached) {
  if (!Parser::Open(path.string(), std::ifstream::in | std::ios::binary)) {
    Logger::error(PARSERS) << "Unable to open Gm1 file '" << path.string()
                           << "'!" << std::endl;
    return false;
  }
  ...
  if (header.num > MAX_NUM) {
    Logger::error(PARSERS) << "Gm1 file header from '" << path.string()
                           << "' contains too many images!" << std::endl;
    Parser::Close();
    return false;
  }
  ...
  if (header.type == Gm1Header::TYPE_TILE) {
    ...
    for (n = 0; n < numCollections; n++) {
      ...
      /* Calculate the offset relative to the texture atlas rectangle for every
       * entry */
      for (uint32_t d = currentEntry + cnt; currentEntry < d; currentEntry++) {
        entries[currentEntry].collection = n;
        entries[currentEntry].offX =
            int(entries[currentEntry].header.offsetX +
                entries[currentEntry].header.horizOffset) -
            left;
        entries[currentEntry].offY =
            int(entries[currentEntry].header.offsetY) - top;
        entries[currentEntry].tileX =
            int(entries[currentEntry].header.offsetX) - left;
        entries[currentEntry].tileY =
            int(entries[currentEntry].header.offsetY +
                entries[currentEntry].header.tileOffsetY) -
            top;
      }

      /* Hand the width/height to the texture atlas  */
      entryDims[n] = std::make_pair<uint32_t, uint32_t>(uint32_t(right - left),
                                                        uint32_t(bottom - top));
    }
...
bool Gm1File::GetImage(uint32_t index, std::vector<Gm1Entry> &entries,
                       char *imgdata, Gm1Header *header) {
  /* Seek to position */
  ...
  switch (header->type) {
    ...
    case Gm1Header::TYPE_CONSTSIZE: {
      ...
      TgxFile::ReadTgx(textureAtlas->GetSurface(), position,
                       entries[index].size, part.x, part.y, nullptr, 0);
    } break;
    case Gm1Header::TYPE_ANIMATION: {
      SDL_Rect part = textureAtlas->Get(index);
      TgxFile::ReadTgx(textureAtlas->GetSurface(), position,
                       entries[index].size, part.x, part.y, palette,
                       entries[index].header.color % 10);
    } break;
    case Gm1Header::TYPE_TILE: {
      ...
      TgxFile::ReadTgx(textureAtlas->GetSurface(), position + 512,
                       entries[index].size - 512, part.x + entries[index].offX,
                       part.y + entries[index].offY, nullptr, 0);

      /* Extract pixel data */
      const static uint8_t lines[16] = {2,  6,  10, 14, 18, 22, 26, 30,
    ...
    default: {
      Logger::error(PARSERS)
          << "Unknown filetype stored in Gm1: " << header->type << std::endl;
      return false;
    }
  }
