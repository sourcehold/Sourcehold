...
void System::LoadModCampaigns(const ghc::filesystem::path& base)
{
    ...
    for(const auto &folder : folders) {
        ...
        if(!DoesFileExist(maps) || !IsFolder(maps) ||
           !DoesFileExist(manifest) ||
           !DoesFileExist(campaign)) {
            Logger::error(GAME) << "Campaign at '"
                                << folder
                                << "' does not contain all the necessary files! Please check the wiki!"
                                << std::endl;
            return;
        }
        ...
        try {
            ...
            Logger::success(GAME) <<
                "Registered campaign: '" << info.title << "' by " << info.author << "!" << std::endl;
            ...
            if(info.type == 0) {
                ...
                Logger::warning(GAME) << "Campaign '" << info.title << " has an invalid type!" << std::endl;
            }
        } catch(std::exception &ex) {
            Logger::warning(GAME) << "Failed to read campaign manifest: '" << folder << "'!" << std::endl;
        }
    }
}
