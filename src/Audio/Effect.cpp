...
Effect::Effect() {
  ...
  if (!ic) {
    Logger::error(AUDIO) << "Unable to allocate input format context!"
                         << std::endl;
  }
}
...
bool Effect::Load(ghc::filesystem::path path, bool looping) {
  ...
  int out =
      avformat_open_input(&ic, path.string().c_str(), GetAVInputFormat(), NULL);
  if (out < 0) {
    return false;
  }
  ...
  */
  ...
  while (ret >= 0) {
    ...
    if (!audioInit) {
      ...
      alSampleRate = 22050;
      size = alNumChannels * audioFrame->nb_samples *
             av_get_bytes_per_sample((AVSampleFormat)audioFrame->format);
      audioBuffer = (char*)std::malloc(size);

      swr = swr_alloc_set_opts(
          NULL, AV_CH_LAYOUT_MONO, AV_SAMPLE_FMT_S16, alSampleRate,
          /*audioFrame->channel_layout*/ AV_CH_LAYOUT_MONO,
          (AVSampleFormat)audioCtx->sample_fmt, audioCtx->sample_rate, 0, NULL);
      if (!swr) {
        Logger::error(AUDIO)
            << "Unable to allocate swresample context!" << std::endl;
        return;
      }

      if (swr_init(swr) < 0) {
        Logger::error(AUDIO)
            << "Unable to init swresample context!" << std::endl;
        return;
      }
    ...
    if (alNumFreeBuffers > 0) {
      ...
      int smp = swr_convert(swr, (uint8_t**)audioBuffer, size,
