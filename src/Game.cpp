...
static int MainLoop(UIState state) {
  switch (state) {
    case MAIN_MENU: {
...
static const int resolutions[][2] = {{800, 600},   {1024, 768},  {1280, 720},
                                     {1280, 1024}, {1366, 768},  {1440, 900},
                                     {1600, 900},  {1600, 1200}, {1680, 1050},
                                     {1920, 1080}, {-1, -1}};

static int StartGame(GameOptions& opt) {
  // Init logger //
...
static cxxopts::Options createCLIConfig(GameOptions& opt) {
  ...
  config.add_options()("h,help", "Print this info")("v,version",
      ...
      value<std::string>()->default_value("../data/"))(
      "debug", "Print debug info", value<bool>(opt.debug))(
      "c,color", "Force color output")("f,fullscreen", "Run in fullscreen mode",
                                       value<bool>(opt.fullscreen))(
      "r,resolution", "Resolution of the window",
      value<std::string>()->default_value("1280x720"))(
      "d,disp", "Index of the monitor to be used",
      value<uint16_t>()->default_value("0"))(
      "s,skip", "Skip directly to the main menu", value<bool>(opt.skip))(
      "noborder", "Remove window border", value<bool>(opt.noborder))(
      "nograb", "Don't grab the mouse", value<bool>(opt.nograb))(
      "nosound", "Disable sound entirely", value<bool>(opt.nosound))(
      "nothread", "Disable threading", value<bool>(opt.nothread))(
      "nocache", "Disable asset caching", value<bool>(opt.nocache));
...
static void setColor(cxxopts::ParseResult& parse_result, GameOptions& opt) {
  if (parse_result.count("color") > 0)
    opt.color = parse_result["color"].as<bool>();
  else
    opt.color = -1;
}

static void setDisplay(cxxopts::ParseResult& parse_result, GameOptions& opt) {
  opt.ndisp = parse_result["disp"].as<uint16_t>();
}

static void setDataDir(cxxopts::ParseResult& parse_result, GameOptions& opt) {
  opt.dataDir = parse_result["path"].as<std::string>();
}
...
#else
  const std::string str = parse_result["resolution"].as<std::string>();
...
static void handleOptions(cxxopts::ParseResult& parse_result,
                          GameOptions& opt) {
  setColor(parse_result, opt);
  setDisplay(parse_result, opt);
  setDataDir(parse_result, opt);
  setResolution(parse_result, opt);
}

// This may exit the application on some flags
static GameOptions ParseCommandline(int argc, char** argv) {
  using namespace cxxopts;
  try {
    GameOptions opt;

    auto config = createCLIConfig(opt);

    auto result = config.parse(argc, argv);

    handleExitOptions(result, config);
    handleOptions(result, opt);
