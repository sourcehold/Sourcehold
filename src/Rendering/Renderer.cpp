...
void Rendering::SetTarget(Texture *target, Rect<int> rect)
{
    _tr = rect;
    _target = target;
    if (SDL_SetRenderTarget(_renderer, target ? target->GetTexture() : nullptr) < 0) {
#if 0
...
void Rendering::RenderLine(Line<int> line, Uint8 r, Uint8 g, Uint8 b)
{
    ...
    SDL_RenderDrawLine(_renderer, _tr.x+line.x1, _tr.y+line.y1, _tr.x+line.x2, _tr.y+line.y2);
}
