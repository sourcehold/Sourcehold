...
TEST(SDLBackend, Init) {
  EXPECT_NO_THROW(SDLBackend::Instance());
  EXPECT_NO_THROW(SDLBackend::Renderer());
  EXPECT_NO_THROW(SDLBackend::Window());

  EXPECT_NE(SDLBackend::Renderer(), nullptr);
  EXPECT_NE(SDLBackend::Window(), nullptr);
}
...
TEST(SDLHelpers, AtSafe) {
  ...
  EXPECT_DEATH(AtSafe<uint32_t>(surface.get(), pos) = 3, "");
}
